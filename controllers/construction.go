...
// pages under construction are described by comment-layout.html
// 	{{.Header}}
// 	{{.LayoutContent}}
// 	{{.Footer}}
...
func (cp *ConstructionController) activeContent(view string) {
	cp.Layout = "comment-layout.html"
	cp.LayoutSections = make(map[string]string)
	cp.LayoutSections["Header"] = "header.html"
	cp.LayoutSections["Footer"] = "footer.html"
	cp.TplName = view + ".html"
